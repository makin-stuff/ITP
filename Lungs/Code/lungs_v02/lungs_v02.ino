/*
  ...
  It gets the current AQI data every 15 minutes and a watch dog timer is implemented
...
void loop() {
  ...
  // We want to update the air quality data approx every 15 minutes because it doesn't update that frequently!
  // 90 * 10 second loop = 
  if (loopCount % 45 == 0) {
    currAqi = getAQI();
    Serial.print("AQI: ");
